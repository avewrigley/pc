...
def read_bank_file():
    ...
    if exists(BANK_FILE):
        with open(BANK_FILE, 'r', encoding="UTF-8") as filehandle:
            bank_days = int(filehandle.readline())
            ...
            if start_day_str:
                ...
                start_day = None
            return bank_days, bank, rest_days_used, start_day
    return 0, 0, 0, None


def write_bank_file(days, bank, rest_days_used, start_day):
    """
...
def main():
    ...
    bank_days, bank, rest_days_used, start_day = read_bank_file()
    if args.start_day:
...
bank={bank}'
    ...
    print('/'.join((str(count), str(rest_days_delta), str(bank))))
    write_bank_file(days, bank, rest_days_used, start_day)
